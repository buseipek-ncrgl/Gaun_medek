...
interface LOAchievementBarChartProps {
  achievements: LOAchievement[];
  /** Sınav geçme puanı (0-100). Yoksa 60 kullanılır. */
  passingThreshold?: number;
}

export function LOAchievementBarChart({ achievements, passingThreshold = 60 }: LOAchievementBarChartProps) {
  ...
  const chartData = achievements.map((achievement) => ({
    ...
    hedef: passingThreshold,
  }));

  const getColor = (value: number) => {
    if (value >= passingThreshold) return "#22c55e"; // green-500
  ...
  return (
    <Card className="border-2 border-[#0a294e]/20 dark:border-slate-700">
      <CardHeader>
        <CardTitle className="text-2xl text-[#0a294e] dark:text-foreground">Öğrenme Çıktıları (ÖÇ) Başarı Oranları</CardTitle>
        <CardDescription className="text-base">
          Her öğrenme çıktısı için ortalama başarı yüzdesi (Tüm öğrenciler için)
        </CardDescription>
      </CardHeader>
      <CardContent>
        <div ref={chartContainerRef}>
          <ResponsiveContainer width="100%" height={400}>
            ...
            <Bar
              dataKey="hedef"
              name={`Hedef Eşik (${passingThreshold}%)`}
              fill="#94a3b8"
              opacity={0.3}
              radius={[8, 8, 0, 0]}
            />
        ...
        <div className="mt-4 flex items-center justify-center gap-6 text-sm">
          <div className="flex items-center gap-2">
            ...
            <span className="text-foreground">Başarılı (≥{passingThreshold}%)</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-4 h-4 rounded bg-red-500"></div>
            <span className="text-foreground">Başarısız (&lt;{passingThreshold}%)</span>
          </div>
        </div>
      </CardContent>
    </Card>
